...
def calculateBetweennessAmongCommunity(g,mode="ALL"):
    ...
    for community in uniqueCommunities:
        communityNodes = np.where(np.array(Ci)==community)[0]
...
label2Communities = {}
for networkName,filename in tqdm(networks.items()):
    networkPath = networkFolder/f"{filename}.xnet"
    g = xn.load(networkPath)
    label2Communities[networkName] = dict(zip(g.vs["Label"],g.vs["community"]))
    



for filename in tqdm(networks.values()):
    print(filename)
    networkPath = networkFolder/f"{filename}.xnet"
    g = xn.load(networkPath)
    df = pd.DataFrame()
    ...
    gME = ig.Graph(g.vcount(),connections, directed=False)
    ...
    # save a version of the network with all these extra parameters
    for key in df.columns:
        g.vs[key] = df[key]
    # transfer other communities
    for communityNetworkName,label2community in label2Communities.items():
        g.vs[f"Community_{communityNetworkName}"] = [label2community.get(label,None) for label in g.vs["Label"]]
    xn.save(g, dataFolder/f"{filename}_attributes.xnet")
